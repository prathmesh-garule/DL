...
# Image dimensions
original_img_height, original_img_width = 128, 128  # Original image size
img_height, img_width = 224, 224  # Model's required input size
batch_size = 32

# Data augmentation for training set with image resizing
train_datagen = ImageDataGenerator(
    rescale=1./255,
    shear_range=0.2,
    zoom_range=0.2,
    horizontal_flip=True,
    preprocessing_function=lambda img: tf.image.resize(img, (img_height, img_width))
)

# Rescaling for validation and test sets with image resizing
...
train_generator = train_datagen.flow_from_directory(
    train_path,
    target_size=(original_img_height, original_img_width),  # Original image size
